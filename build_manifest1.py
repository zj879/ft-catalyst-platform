...
def discover_pdfs(root: Path) -> list[tuple[str, Path]]:
    ...
    for source_set, rel_dir in PDF_DIRS:
        pdf_dir = root / rel_dir
        if not pdf_dir.exists():
            continue
        for path in sorted(pdf_dir.glob("*.pdf")):
            pdfs.append((source_set, path))
    return pdfs
...
def main() -> None:
    ...
    parser.add_argument("--out-dir", default=r"D:\Agent\Codex\files\H_ywj_95\saf_prompt_pipeline\outputs")
